...
public class BST<Key extends Comparable<Key>> {
    ...
    public Double averageDepth() {
        return totalDepth(root) * 1.0 / size(root);
    }

    private int totalDepth(Node x) {
        if (size(x) <= 1) { // x has no child
            return 0;
        } else {
            return size(x.left) + size(x.right) + totalDepth(x.left) + totalDepth(x.right);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        temp.add(3);
        temp.add(6);
        System.out.println(temp.size() + " : " + temp.averageDepth());
        temp.add(9);
        temp.add(5);
        temp.add(4);
        temp.add(2);
