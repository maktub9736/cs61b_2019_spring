...
public class SimpleOomage implements Oomage {
    ...
    @Override
    public boolean equals(Object o) {    // DONE: Write this method.
    ...
    public int hashCode() {
        ...
        } else {                        // DONE: Write a perfect hash function for Simple Oomages.
            return helpHash(red, green, blue);
        }
    }

    private int helpHash(int r, int g, int b) {
        int sum = 0;
        int[] colors = {r / 5, g / 5, b / 5};
        int x = 1;
        for (int i = 0; i < 3; i++) {
            sum += x * colors[i];
            x = x * 7;
...
//        randomSimpleOomage().draw(0.75, 0.25, 1);
        System.out.println((5 & 0x7FFFFFFF) % 5);
    }
